...
class AsyncEngineDrivenTransferContext(EngineDrivenTransferContext):
    ...
    def _release_staging(self, chunks: list[torch.Tensor]) -> None:
        ...
        Chunks are bucketed by ``(shape, dtype)`` before being returned:
        a multi-group store stages one buffer set per LMCache group, and
        groups with differing layer counts have differing chunk shapes, so
        a single pool key derived from ``chunks[0]`` would file mismatched
        tensors under the wrong bucket.
        ...
        try:
            ...
            def _prepare_gather_and_commit() -> None:
                ...
                try:
                    ...
                    # SHM path gathers directly into the server's SHM views;
                    # the pickle path stages into pinned buffers that must be
                    # returned to the pool afterwards. The two are mutually
                    # exclusive for a single store, so latch the mode once
                    # here rather than inside the per-group loop below.
                    ...
                    # group-major (matching the order
                    # EngineDrivenTransferModule.prepare_store concatenated
                    # them in); each group's own chunk-count range is sliced
                    # out before gathering that group.
