...
def _select_kv_caches_by_index(
    ...
    Called from ``_kv_caches_for_group`` (register-time, per group) and
    ``GroupTransferPlan.select_kv_caches`` (per transfer, using the cached
    ``layer_indices``).
...
def _kv_caches_for_group(
    ...
    Called from ``_build_group_transfer_plans``, once per group at
    ``register()`` time, to detect that group's own KV format.
...
def _blocks_per_chunk_for_group(
    ...
    Called from ``_build_group_transfer_plans``, once per group at
    ``register()`` time, to cache each plan's ``blocks_per_chunk``.
...
def _group_chunk_shape(
    ...
    Called from ``_build_group_transfer_plans``, once per group at
    ``register()`` time, to cache each plan's ``chunk_shape``.

    Substitutes the group's own layer count for ``num_layers``, and, for
    non-recurrent groups, its own ``hidden_dim_size`` -- hybrid groups can
    pack K/V differently (e.g. sliding-window fused, full-attention split),
    so reusing the default group's width can under- or over-size this
    group's SHM chunk buffer.

    Recurrent-state groups (Mamba/GDN) are excluded from the hidden_dim
    recompute: their tensor has already been rewritten by a synthetic
    addressing view (e.g. ``_MambaPageViewEdit``) whose ``(heads,
    head_size)`` split only makes the byte count line up with the
    conv/ssm/pad page layout and carries no real attention semantics.
...
class GroupTransferPlan:
    ...
    Everything a per-group transfer needs except the KV tensors and the
    request's block IDs is fixed once the worker registers: the group's
    layer selection, its blocks-per-chunk count, its chunk shape, and its KV
    format. Recomputing them per transfer made the hot path
    ``O(num_groups * num_layers^2)`` plus one full
    :func:`~lmcache.v1.gpu_connector.utils.normalize_kv_and_discover_format`
    per group, so :meth:`EngineDrivenTransferContext.register` resolves them
    once and every later transfer reuses them.

    The KV tensors themselves are deliberately *not* cached: retrieve
    scatters into whatever mapping the caller passes, which is not
    necessarily the one registered. :meth:`select_kv_caches` applies the
    cached layer selection to the caller's mapping instead.
...
class GroupChunkSelection:
    ...
    The server hands the worker a single ``chunk_indices`` list (and, on the
    SHM path, a matching ``out_buffers`` list) covering every group's chunks
    group-major. Gathering a group needs those re-expressed relative to that
    group alone.
...
def null_chunk_mask_from_groups(
    ...
    vLLM nulls out-of-window block-table entries (id ``0``) for both
    recurrent groups (align-mode Mamba/GDN: only the trailing state block is
    resident) and sliding-window attention groups (prefix outside the
    window). Those chunks carry no valid KV and must not be stored, so every
    group is checked the same way regardless of kind (mirrors the CUDA-IPC
    path's ``all_null_chunk_masks``). Full-attention blocks are never nulled,
    so they always yield an all-``False`` mask.
...
def _detect_group_kv_format(
    ...
    Hybrid models may mix per-group KV formats (e.g. attention + mamba
    views), so each group's format is discovered from its own tensor subset
    rather than reusing the model-wide value. A group whose subset does not
    match any known format yields ``None``, which makes gather / scatter fall
    back to detecting it per transfer -- the pre-cache behaviour, so a group
    we cannot pre-resolve still transfers correctly (just without the saved
    detection).
...
def _build_group_transfer_plans(
    ...
    Called from :meth:`EngineDrivenTransferContext.register`. See
    :class:`GroupTransferPlan` for why these are cached rather than derived
    per transfer.
...
class EngineDrivenTransferContext(TransferContext):
    ...
    def iter_transfer_groups(
        ...
        Exposed so subclasses (e.g.
        :class:`AsyncEngineDrivenTransferContext`) can iterate per group
    ...
    def register(
        ...
        ``engine_group_infos`` is used to split worker-side gather / scatter
        by LMCache group at store / retrieve time, matching the CUDA transfer
        path's per-group block-id addressing. An empty sequence registers a
        single non-hybrid group. Each group's transfer constants -- its tensor
        subset, blocks-per-chunk, chunk shape and KV format -- are resolved
        once here into :class:`GroupTransferPlan` entries and reused by every
        subsequent transfer (see :meth:`iter_transfer_groups`).

        ``engine_type`` is accepted to satisfy the base interface but is
        unused: only the handle path consumes it.
        """
        ...
        # Each plan's chunk_shape already carries this group's own
        # hidden_dim_size (see _group_chunk_shape) -- read it back out so the
        # server can size each group's own SHM chunk buffer instead of
        # reusing the single shared hidden_dim_size for every group.
    ...
    def _has_maskable_group(self) -> bool:
        ...
        Both recurrent-state groups and genuine sliding-window attention
        groups have vLLM null out block-table entries outside their live
        window (see :func:`null_chunk_mask_from_groups`); full-attention
        groups never do. Gates the (otherwise unconditional) null-chunk-mask
        computation so a store/retrieve with only full-attention groups
        skips it.
        """
        return any(
    ...
    def submit_store(
        ...
        # whole multi-group chunk sequence, group-major (group 0's chunks
        # first, then group 1's, ...), matching the order the server
        # concatenated them in (see
        # ``EngineDrivenTransferModule.prepare_store``). Each group's own
    ...
    def submit_retrieve(
        ...
        if src_buffers is not None:
            try:
                ...
                # it in (see the group_offset bookkeeping there), minus any
                # null chunks the server excluded (see
                # ``EngineDrivenTransferModule.prepare_retrieve``): those
                # chunks were never stored, so their positions are dropped
                # from ``src_buffers`` rather than filled with data.
                group_offset = 0
